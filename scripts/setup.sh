...
# NOTE: libopencv-dev removed — opencv-python-headless pip package is sufficient
apt-get install -y -q \
...
apt-get install -y -q htop nvtop nodejs npm libopencv-dev || log "Some optional packages failed, continuing"
